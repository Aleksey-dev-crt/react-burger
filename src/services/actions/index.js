import {
  SET_LOADER_ON_INGREDIENTS,
  SET_LOADER_ON_POST_ORDER,
  SET_CATEGORY, 
...
export const requestIngredients = () => {
  return (dispatch) => {
    dispatch({ type: SET_LOADER_ON_INGREDIENTS, payload: true })
    getIngredients()
      .then((res) => dispatch({ type: MODIFY_INGREDIENTS, payload: res.data }))
      .finally(() => dispatch({ type: SET_LOADER_ON_INGREDIENTS, payload: false }))
...
export const postOrder = (payload) => {
  return (dispatch) => {
    dispatch({ type: SET_LOADER_ON_POST_ORDER, payload: true })
    placeOrder(payload)
